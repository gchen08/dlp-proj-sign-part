/**   
* @Title: XlsxFormat.java
* @Package  team.sjtu14573.antileak.injection
* @Description: Xlsx格式文件嵌入/提取程序
* @author gchen08   
* @date 2016/7/28
* @version V1.0   
*/
	...
	*/
	public static boolean embedProcess(String filePath, String tag)
	{
		try
		{
			...
			Document xmlDocument = db.parse(new File(filePath + "xl/workbook.xml"));
			Node node = xmlDocument.getElementsByTagName("sheet").item(0);
			...
			FileOutputStream fos = new FileOutputStream(filePath + "xl/workbook.xml");
	...
	*/
	public static String extractProcess(String filePath, int len)
	{
		try
		{
			...
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document xmlDocument = db.parse(new File(filePath + "xl/workbook.xml"));
			NodeList nList = xmlDocument.getElementsByTagName("sheet");
